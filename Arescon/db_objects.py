...
# TODO вместо функций организации SSH добавить объект класса SSH
    def __init__(self, start_date: datetime, end_date: datetime, name_agent=None, post_working=False, ts_name="Приват Сквер"):
        ...
        self._db_password = ''
        self._db_mysql_url = '185.148.37.101'
        self._db_mysql_port = 63306
        self._db_mysql = 'processing'
        ...
        self._COLUMNS_RESULT = ['Квартира',	'Услуга', 'Модель',	'№ ПУ',	'Начальная дата', 'Конечная дата',
                                'Начало',	'Конец',	'Потребление',	'ext_id']
        self._sort_column = 'Квартира'
    ...
    def open_pa3_db(self):
        is_connection_error = True
        while is_connection_error:
            try:
                self.open_ssh()
                pa3_db = pw.PostgresqlDatabase(self._psql_name, host=self._psql_url, port=self._server.local_bind_port,
                                               user=self._psql_login, password=self._psql_psw)
                pa3_database_proxy.initialize(pa3_db)
                is_connection_error = False
            except OperationalError:
                time.sleep(30)

    def open_dcs_db(self):
        is_connection_error = True
        while is_connection_error:
            try:
                dcs_db = pw.MySQLDatabase(self._db_mysql, host=self._db_mysql_url, port=self._db_mysql_port,
                                           user=self._db_username, password=self._db_password)
                dcs_database_proxy.initialize(dcs_db)
                is_connection_error = False
            except OperationalError:
                time.sleep(30)
    ...
    def get_datequery(self):
        start_date = self._start_date - timedelta(self._period_exam * 30)
...
# Create an SSH server
    ...
    def close_all_db(self):
        self.close_ssh()
        pa3_database_proxy.close()
        dcs_database_proxy.close()
...
# загрузить счетчики с последней калибровкой
    def get_counters_lk3(self):
        self.open_pa3_db()
        return Meters.get_counters_by_tszh(self._tszh_name)
    ...
    def get_terminals(self, include_term: list = None):
        if self._is_sql:
            sql_engine = create_engine(
                'mysql+pymysql://' + self._db_username + ':' + self._db_password + '@' + self._db_mysql_url + ':'
                + str(self._db_mysql_port) + '/' + self._db_mysql, pool_recycle=3600)
...
# загрузить все терминалы с данными по датам данных методом peewee
    def get_terminals_peewee(self) -> pd.DataFrame:
        data_set = TerminalsData.select().where(TerminalsData.stamp.between(self._start_date, self._end_date))
        return pd.DataFrame(list(data_set.dicts()))

    @staticmethod
    def get_terminals_by_ext_id(ext_id) -> pd.DataFrame:
        data_set = Terminals.select().where(Terminals.ext_id == ext_id)
        return pd.DataFrame(list(data_set.dicts()))

# загрузить все терминалы с данными по датам данных
    def get_alarms(self):
        return TerminalsData.get_by_dates_port(self._start_date, self._end_date, 3)
...
# выгрузка данных в файл
    ...
    def merge(self):
        ...
        # создание df merge
        df_merge = df_load.merge(df_calc, on='ext_id', how='left')
...
# данные из столбца "Конец" переписать в столбец "Начало", в столбец "Конец" записать сумму "Начало" и res_cons
        df_result['Квартира'].astype(str)
        df_result.loc[df_result['cons'].isna(), 'cons'] = 'нет данных'
        df_result.loc[df_result['Конец'].isna(), 'Конец'] = 'нет данных'
        df_result['Потребление'] = df_result['cons']
        df_result['Начало'] = df_result['Конец']

        criteria = (df_result['Начало'].ne('нет данных')) & (df_result['cons'].ne('нет данных'))
        df_result['Конец'] = df_result.loc[criteria, ['Начало', 'cons']].sum(axis=1)
        df_result.loc[df_result['Конец'].isna(), 'Конец'] = 'нет данных'
...
#         df_result['Лицевой счет'] = df_result['Квартира'].str.extract(r'(\s\d+\s*$)')
#         df_result['Префикс'] = self._prefix_account
#         criteria = ~df_result['Квартира'].isna() & (df_result['Квартира'].str.len() == 2)
#         df_result.loc[criteria, 'Префикс'] = self._prefix_account[:len(self._prefix_account)-1]
#         criteria = ~df_result['Квартира'].isna() & (df_result['Квартира'].str.len() == 3)
#         df_result.loc[criteria, 'Префикс'] = self._prefix_account[:len(self._prefix_account)-2]
#         criteria = df_result['Лицевой счет'].isna() & ~df_result['Квартира'].isna()
#         df_result.loc[criteria, 'Лицевой счет'] = df_result.loc[criteria, 'Префикс'] + df_result.loc[criteria, 'Квартира']
...
class GetDataPrivate(AresconData):
    ...
    def __init__(self, start_date: datetime, end_date: datetime):
        ...
        self._ssh_psw = 'iu97GHd%g'
        self._db_mysql_url = '94.228.246.228'
        self._COLUMNS_RESULT = ['Корпус', 'Квартира',	'Услуга', 'Модель',	'№ ПУ',	'Начальная дата', 'Конечная дата',
...
class GetDataKvart23(AresconData):
    ...
    def __init__(self, start_date: datetime, end_date: datetime):
        ...
        self._db_mysql_url = '46.39.252.67'
        # инициация Postgres БД ('private')
...
class GetDataKvart126(AresconData):
    def __init__(self, start_date: datetime, end_date: datetime):
        ...
        self._db_mysql_url = '185.148.37.101'
        self._db_mysql = 'processing_kvart'
    # инициация Postgres БД ('private')
...
def get_agent(start_date: str, end_date: str, name_agent: str):
...
@dispatch(datetime, datetime, str)
def get_agent(start_date: datetime, end_date: datetime, name_agent: str):
    if name_agent == 'private':
        obj = GetDataPrivate(start_date, end_date)
    elif name_agent == 'kvart23':
        obj = GetDataKvart23(start_date, end_date)
...
@dispatch(datetime, float, str)
def get_agent(start_date: datetime, tdelta: float, name_agent: str):
    end_date = start_date + timedelta(hours=tdelta)
    if name_agent == 'private':
        obj = GetDataPrivate(start_date, end_date)
    elif name_agent == 'kvart23':
        obj = GetDataKvart23(start_date, end_date)
...
@dispatch(float, datetime, str)
def get_agent(tdelta: float, end_date: datetime, name_agent: str):
    start_date = end_date - timedelta(hours=tdelta)
    if name_agent == 'private':
        obj = GetDataPrivate(start_date, end_date)
    elif name_agent == 'kvart23':
        obj = GetDataKvart23(start_date, end_date)
